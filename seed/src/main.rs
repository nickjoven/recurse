...
fn main() {
    ...
    let predictions: Vec<PredictionEntry> = vec![
        ...
        PredictionEntry {
            ...
            name: "Hawking temperature from tongue-edge gradient [GAP: missing reduction]",
            description: "Predicts T_H = (ℏ/2πk_B) · |dρ/dΩ| at the tongue edge. DEPENDENCY GAP: requires showing that Munoz-Arboleda et al.'s non-Hermitian tight-binding Hamiltonian (2507.03826) reduces to a circle map under some parameter mapping. This is plausible — non-Hermitian lattice models can relate to kicked rotors via Floquet analysis — but the reduction has not been performed. Without it, the prediction is a formula without a bridge to the system it claims to describe. The intermediate step (lattice Hamiltonian → circle map → tongue boundary → rotation number gradient) is a paper-sized problem, not a one-line derivation.",
            quantity: "T_Hawking",
            predicted_value: "(hbar / 2 pi k_B) * |d rho / d Omega|_{tongue edge}",
            source_derivation: "analogue_horizon → arnold_tongues (GAP: lattice-to-circle-map reduction not performed)",
            testable_by: "First: derive circle-map reduction of the non-Hermitian lattice Hamiltonian. Then: compare T_H from both formalisms.",
        ...
        PredictionEntry {
            ...
            name: "Vortex array stability from Farey depth [GAP: no supporting data]",
            description: "CONJECTURE — NO DATA. Hypothesizes that a lattice of N vortices is stable iff all nearest-neighbor topological charge pairs are Farey neighbors (|ℓ_i·q_j - ℓ_j·q_i| = 1). Maximum stable array size at Stern-Brocot depth D would equal |F_D|. Status: purely speculative. Paper 8 (Fan et al. 2512.16308) demonstrates holographic control over polygonal STOV geometry but tests no stability thresholds and invokes no Farey structure. This prediction has zero empirical or theoretical grounding beyond analogy. EXCLUDED FROM ACTIVE PREDICTIONS — retained only as a research direction to revisit if stability data becomes available.",
            quantity: "N_max_stable",
            predicted_value: "UNGROUNDED — |F_D| conjectured, not derived",
            source_derivation: "vortex_array → farey_graph (GAP: no intermediate derivation exists)",
            testable_by: "Requires systematic vortex array stability measurements that do not yet exist",
            parent_slugs: &["vortex-array", "farey-graph", "stern-brocot-tree"],
        },
        PredictionEntry {
            slug: "circuit-depth-sb-depth",
            name: "Quantum circuit depth equals Stern-Brocot depth [GAP: KAM bridge needed]",
            description: "Predicts minimum circuit depth ∝ depth_SB(p/q) for target rotation number precision. DEPENDENCY GAP: requires proving that point-vortex Hamiltonian eigenvalues are organized by rational rotation numbers — a KAM theory question for the specific system Wang et al. (2506.04023) simulate. Their paper reformulates Navier-Stokes as a quantum Hamiltonian for multi-vortex dynamics; it does not address rotation numbers, circle maps, or Stern-Brocot structure. The bridge from 'Hamiltonian vortex dynamics' to 'Stern-Brocot depth governs complexity' depends on an unstated theorem about integrability of the point-vortex system on 8 qubits.",
            quantity: "D_circuit",
            predicted_value: "depth_SB(p/q) where |rho - p/q| < epsilon (GAP: see description)",
            source_derivation: "quantum_vortex_simulation → stern_brocot_tree (GAP: KAM theorem for point-vortex Hamiltonians not established)",
            testable_by: "First: establish rotation-number structure in point-vortex Hamiltonian spectrum. Then: benchmark circuit depth vs. SB depth.",
