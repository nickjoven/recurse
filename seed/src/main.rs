...
fn main() {
    ...
    let predictions: Vec<PredictionEntry> = vec![
        ...
        PredictionEntry {
            ...
            testable_by: "Map Munoz-Arboleda et al. (2507.03826) lattice parameters to circle-map params; compare T_H from both formalisms",
            parent_slugs: &["analogue-horizon", "rotation-number", "arnold-tongues"],
        },
        PredictionEntry {
            slug: "combining-efficiency-scaling",
            name: "Combining efficiency follows tongue-width decay",
            description: "The combining efficiency of coherent beam combining for charge-ℓ vortex beams follows η(ℓ) = 1 - c·(K/2)^ℓ for some coupling K and constant c. The (K/2)^ℓ factor is the Arnold tongue width at topological charge ℓ. Fathi et al. (2512.19815) measured η = 95.0%, 93.9%, 91.2% at ℓ = 1, 5, 8 respectively — the decreasing trend is consistent with tongue-width scaling but requires fitting K and c, then predicting η for untested charges.",
            quantity: "eta(ell)",
            predicted_value: "1 - c * (K/2)^ell",
            source_derivation: "vortex_array → arnold_tongues → tongue_width_scaling",
            testable_by: "Coherent beam combining efficiency at ℓ = 2, 3, 4 (Fathi et al. 2512.19815 measured ℓ = 1, 5, 8)",
        ...
        PredictionEntry {
            ...
            testable_by: "Benchmark Wang et al. (2506.04023) 8-qubit circuit depth vs rotation-number precision against Stern-Brocot depth",
