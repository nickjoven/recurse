...
fn main() {
    ...
    let ontology: Vec<OntologyEntry> = vec![
        ...
        OntologyEntry {
            ...
            description: "INTERPRETATION: proposes that the boundary of an Arnold tongue can be reinterpreted as an event horizon — mode-locked (subsonic) inside, drifting (supersonic) outside. Grounded: analogue BH systems (Munoz-Arboleda et al. 2507.03826) do create horizons at interfaces between distinct propagation regimes. Interpretive: the identification of tongue boundary = horizon, and T_H ∝ |dρ/dΩ|, is our framework's proposal, not established physics. The Bekenstein-Hawking entropy → Stern-Brocot address count is speculative.",
        ...
        OntologyEntry {
            ...
            description: "Grounded: near a phase singularity (intensity zero), the Poynting vector locally reverses direction — demonstrated by Kotlyar et al. (Opt. Lett. 2026) and Pryamikov (2601.21704). Gigantic wave vectors of both signs appear near the zero. INTERPRETATION: the cost-functional saddle structure and the continued-fraction/superoscillation parallel are our framework's reading. The figure-eight (Klein bottle) connection is speculative — the papers attribute reversal to non-paraxial field structure and Poynting-vector vortex geometry, not to topological surface immersions.",
        ...
        OntologyEntry {
            ...
            slug: "vortex-array",
            name: "Vortex array",
            description: "Grounded: coherent beam combining produces high-power charge-ℓ vortices (Fathi et al. 2512.19815, 100W/100kW at ℓ=1,5,8). Polygonal STOV wavepackets with prescribed singularity arrangements exist (Fan et al. 2512.16308). INTERPRETATION: the claim that combining efficiency scales as (K/2)^ℓ tongue width is confounded — efficiency loss may be dominated by phase-error accumulation, not tongue structure (see combining-efficiency-scaling prediction). The Farey neighbor stability claim has no data. The Farey graph tiling of polygonal configurations is speculative — holographic shaping prescribes geometries without invoking number-theoretic constraints.",
            symbol: None,
        ...
        OntologyEntry {
            ...
            description: "Grounded: Wang et al. (2506.04023) reformulate Navier-Stokes as a quantum Hamiltonian and simulate multi-vortex interactions on 8 qubits (99.97%/99.76% fidelity). INTERPRETATION: the claim that qubits 'natively encode circle-map dynamics' is geometrically motivated (Bloch sphere ⊃ S¹) but not how the paper frames it — they use a Hamiltonian simulation approach, not a circle-map encoding. The conservation-computability link is our framework's. The circuit-depth ∝ Stern-Brocot-depth prediction requires a KAM bridge theorem that has not been established.",
            symbol: None,
