...
fn main() {
    ...
    let ontology: Vec<OntologyEntry> = vec![
        ...
        OntologyEntry {
            ...
            slug: "integers",
            name: "Integers",
            description: "The discrete substrate. Z as the free group on one generator.",
            symbol: Some("Z"),
        ...
        OntologyEntry {
            ...
            description: "Every real number has a unique continued fraction [a₀; a₁, a₂, ...]. The partial convergents trace the Stern-Brocot path. Rational numbers terminate; irrationals do not. The golden ratio φ = [1;1,1,...] is the most poorly approximable — slowest to converge.",
            symbol: Some("[a₀; a₁, a₂, ...]"),
        ...
        OntologyEntry {
            ...
            slug: "golden-ratio",
            name: "Golden ratio φ",
            description: "φ = (1+√5)/2 ≈ 1.618. The fixed point of x ↦ 1+1/x. Continued fraction [1;1,1,...] — all 1s. The noble number: hardest to mode-lock. The rotation number that organizes the devil's staircase.",
            symbol: Some("φ"),
        ...
        OntologyEntry {
            ...
            slug: "circle-s1",
            name: "Circle S¹",
            description: "R/Z — the reals modulo 1. Phase is defined on this space. On the circle, fixed points become rotation numbers, and rational and irrational dynamics diverge.",
            symbol: Some("S¹"),
        ...
        OntologyEntry {
            ...
            description: "The complete binary tree of positive rationals generated by iterated mediants from 0/1 and 1/0. Every positive rational appears exactly once. Depth in the tree equals continued fraction length. The tree encodes the rationals in their natural order.",
            symbol: None,
        ...
        OntologyEntry {
            ...
            description: "As the parabola parameter c increases, stable fixed points bifurcate: period 1→2→4→8→... The ratio of successive bifurcation intervals converges to Feigenbaum's δ. At the accumulation point: chaos. The cascade is universal for all unimodal maps. At K > 1 in the circle map, each Arnold tongue develops its own period-doubling cascade internally — the tongue's attracting orbit destabilizes and cascades to chaos within the tongue.",
            symbol: Some("2^n → chaos"),
            tags: &["derived", "bifurcation", "universal", "route-to-chaos"],
            parent_slugs: &["parabola", "feigenbaum-constants", "arnold-tongues"],
        },

        // ── K > 1 nodes: the overcritical regime ────────────────────────
        // These nodes encode what happens when the circle map folds.
        // The devil's staircase breaks; new structure emerges.

        OntologyEntry {
            level: 1,
            slug: "lyapunov-exponent",
            name: "Lyapunov exponent",
            description: "λ = lim (1/n) Σ log|f'(θᵢ)| — the average rate of divergence of nearby orbits. Replaces the rotation number as the primary diagnostic at K > 1. λ < 0: mode-locked (inside a tongue). λ = 0: critical boundary or quasiperiodic. λ > 0: chaos (sensitive dependence on initial conditions). The graph of λ(Ω) at K > 1 is the successor to the devil's staircase: negative dips on mode-locked islands, positive peaks in chaotic bands, zero at transitions.",
            symbol: Some("λ"),
            tags: &["derived", "dynamics", "chaos", "diagnostic", "overcritical"],
            parent_slugs: &["circle-map", "parabola"],
        },
        OntologyEntry {
            level: 2,
            slug: "multistability",
            name: "Multistability",
            description: "At K > 1, Arnold tongues overlap — a single parameter value (Ω, K) can lie inside multiple tongues simultaneously. The system supports coexisting attractors with different rotation numbers. Which attractor is selected depends on initial conditions and history, not parameters alone. Self-consistency becomes multi-valued. This has no K ≤ 1 analogue; it is a direct consequence of the map losing injectivity (the fold creating multiple preimages).",
            symbol: None,
            tags: &["algebra", "overlap", "coexistence", "history-dependent", "overcritical"],
            parent_slugs: &["arnold-tongues", "lyapunov-exponent"],
        },
        OntologyEntry {
            level: 2,
            slug: "tongue-internal-cascade",
            name: "Tongue-internal period-doubling",
            description: "At K > 1, each Arnold tongue develops internal bifurcation structure. The period-q attracting orbit within the p/q tongue loses stability and undergoes a Feigenbaum cascade: period q → 2q → 4q → 8q → ... → chaos. Each tongue recapitulates the structure of the logistic map. The cascade rate converges to Feigenbaum's δ = 4.6692... universally across all tongues. The Feigenbaum accumulation point marks the onset of chaos within the tongue.",
            symbol: Some("q → 2q → 4q → chaos"),
            tags: &["algebra", "bifurcation", "feigenbaum", "internal", "overcritical"],
            parent_slugs: &["arnold-tongues", "period-doubling-cascade", "feigenbaum-constants"],
        },
        OntologyEntry {
            level: 2,
            slug: "self-similar-descent",
            name: "Self-similar descent",
            description: "Within the chaotic bands at K > 1, periodic windows appear — narrow islands of mode-locking inside chaos. These windows are organized by the same Stern-Brocot hierarchy that organized the original Arnold tongues: the largest windows have the smallest denominator q, and window widths scale with continued-fraction depth. The tree does not vanish at K > 1; it descends, reappearing at every scale within each chaotic region. The Feigenbaum renormalization group is the mechanism of this descent — each period-doubling cascade carries the Stern-Brocot structure to a smaller scale.",
            symbol: None,
            tags: &["algebra", "self-similar", "windows", "renormalization", "overcritical"],
            parent_slugs: &["tongue-internal-cascade", "stern-brocot-tree", "period-doubling-cascade"],
        },
        ...
        OntologyEntry {
            ...
            slug: "field-equation",
            name: "Field equation",
            description: "The synchronization cost functional whose Euler-Lagrange equation gives the field equation. Coupling cost + drift cost, extremized. The Kuramoto-to-Einstein derivation chain (harmonics/sync_cost/derivations/13_einstein_from_kuramoto.md) shows that the continuum limit at K = 1 uniquely produces the Einstein field equations via Lovelock's theorem. The variational principle is developed in harmonics/sync_cost/FRAMEWORK.md.",
            symbol: Some("δS/δg = 0"),
        ...
        OntologyEntry {
            ...
            description: "The immersion of the Klein bottle in R³ produces a figure-eight cross-section. The self-intersection curve is the Möbius band. The non-orientability of this structure relates convergent and residual components of the parameter space.",
            symbol: Some("∞"),
        ...
        OntologyEntry {
            ...
            description: "The empirical question: how many stars land at each Farey fraction? The population N(p/q) at fraction p/q should scale as (K/2)^q if the stellar population follows the circle map dynamics. This quantity bridges the algebraic framework and stellar observation.",
        ...
        OntologyEntry {
            ...
            description: "Analogue BH systems create horizons at interfaces between distinct propagation regimes (Munoz-Arboleda et al. 2507.03826). In the gravity sector, the horizon identification is derived: the Kuramoto-Einstein dictionary gives r = N (coherence = lapse), so the horizon is r = 0 = N = 0, and K(x,x') = G_γ(x,x') is the spatial Green's function (see proslambenomenos/kuramoto_einstein_mapping.md). The grammar classifies horizons as boundaries between λ < 0 (locked) and λ > 0 (chaotic) regions; at K > 1, the horizon has internal structure via tongue-internal cascade. K-mapping status: derived for the gravity sector; open for Paper 1's non-Hermitian lattice, where K(hopping asymmetry, gain/loss) remains to be derived.",
            symbol: None,
        ...
        OntologyEntry {
            ...
            description: "Near phase singularities, the Poynting vector reverses — demonstrated by Kotlyar et al. (Opt. Lett. 2026) and Pryamikov (2601.21704). Fold measure μ = arccos(1/K)/π scales as √(K-1), not (K-1). In the grammar, the fold region {θ: F'(θ)<0} of the overcritical circle map corresponds to the region of reversed flow. For photonic crystals, K likely depends on refractive index contrast, but the mapping K(Δn) is not derived. The fold measure scales as √(K-1) while photonic band gaps scale as Δn, implying K-1 ∝ (Δn)² if the identification holds — but this requires derivation. K-mapping status: open.",
            symbol: Some("F'(θ) < 0"),
        ...
        OntologyEntry {
            ...
            slug: "vortex-array",
            name: "Vortex array",
            description: "Coherent beam combining produces high-power charge-ℓ vortices with efficiency decreasing with ℓ (Fathi et al. 2512.19815: 95.0%, 93.9%, 91.2% at ℓ=1,5,8). Polygonal STOVs with prescribed geometries exist (Fan et al. 2512.16308). At K > 1, the grammar implies multistability (coexisting OAM states) and tongue-internal cascades. The relationship between beam combining efficiency and tongue-width scaling has not been established. K-mapping status: open; K(phase_noise, N_beams, ℓ) for coherent beam arrays remains to be derived.",
            symbol: None,
        ...
        OntologyEntry {
            ...
            description: "Wang et al. (2506.04023) simulate multi-vortex Navier-Stokes dynamics on 8 qubits (99.97%/99.76% fidelity). Multi-vortex interactions produce the mutual coupling, multistability, and chaos characteristic of K > 1 dynamics. In the grammar, simulation difficulty should track the self-similar descent (periodic windows within chaos organized by SB depth). The inter-vortex Hamiltonian H = -(Γ²/4π)Σlog|rᵢ-rⱼ| gives coupling that grows with 1/r², but mapping this to an effective K(Γ, r, N) for the circle-map grammar has not been done. K-mapping status: open.",
            symbol: None,
    ...
    let predictions: Vec<PredictionEntry> = vec![
        PredictionEntry {
            ...
            description: "The CMB spectral index. In the framework, n_s = 1 - 1/φ⁴ where φ = (1+√5)/2, derived from the devil's staircase self-similarity at the golden ratio fixed point. The deviation from scale-invariance corresponds to the fixed-point correction.",
            quantity: "n_s",
        ...
        PredictionEntry {
            ...
            description: "The acceleration below which galactic dynamics deviates from Newtonian gravity. In the framework, a₀ = cH₀/2π, the acceleration at which synchronization coupling cost equals drift cost. If this identification holds, a₀ is determined by the Hubble rate rather than being a free parameter.",
            quantity: "a_0",
        ...
        PredictionEntry {
            ...
            description: "The fraction of the universe's energy density in dark energy. The Farey partition gives Ω_Λ ∈ [13/19, 11/16], where the 13/19 lower bound is the Klein bottle's Farey-6 vertex/edge ratio and the 11/16 upper bound comes from the next Farey order correction.",
        ...
        PredictionEntry {
            ...
            description: "The ratio between the gravitational and electroweak scales. In the synchronization framework, the ratio of coupling constants across the four modes maps to the Stern-Brocot depth separation between the gravitational tongue (largest, shallowest) and the electroweak tongue.",
        ...
        PredictionEntry {
            ...
            description: "The ratio α_s/α₂ at unification. The Klein bottle mode structure assigns coupling strengths in ratio 27/8, emerging from the Stern-Brocot addresses of each mode's tongue.",
        ...
        PredictionEntry {
            ...
            description: "The electroweak mixing angle. The mode partition on the Klein bottle gives sin²θ_W = 8/35, the fraction of the electroweak tongue area assigned to hypercharge vs isospin.",
        ...
        PredictionEntry {
            ...
            description: "Exactly three generations of fermions. The Klein bottle topology gives π₁(K²) = Z ⋊ Z, and the fundamental domain under the modular group has exactly 3 inequivalent cusps, each corresponding to a generation.",
            quantity: "N_gen",
            predicted_value: "3",
        ...
        PredictionEntry {
            ...
            name: "Spatial dimension d=3",
            description: "Three macroscopic spatial dimensions. In the framework, this is derived from the Stern-Brocot branching structure requiring 3 independent directions for consistent embedding, rather than assumed as an input.",
            quantity: "d",
            predicted_value: "3",
        ...
        PredictionEntry {
            ...
            description: "The width of the p/q tongue at coupling K scales as (K/2)^q. This is exact for the standard circle map. Whether it holds for the stellar population distribution is an open question. The exponent q is the Stern-Brocot depth — the continued fraction length.",
            quantity: "W(p/q, K)",
        ...
        PredictionEntry {
            ...
            description: "In the framework, the MOND acceleration scale is related to the cosmological constant by a₀ = c√(Λ/3)/2π, with both emerging from the synchronization cost at different scales. This relation is a consequence of the framework, not an independent input.",
        ...
        // ── Vortex-derived predictions (open) ──────────────────────────
        PredictionEntry {
            slug: "hawking-from-tongue-edge",
            name: "Hawking temperature from tongue-edge gradient",
            description: "Open question: does the non-Hermitian lattice Hamiltonian of Munoz-Arboleda et al. (2507.03826) reduce to a circle map under Floquet analysis? If so, the tongue-edge gradient yields T_H = (hbar/2pi k_B) |d rho/d Omega| at the tongue edge, which can be compared to their Parikh-Wilczek tunneling result. In the gravity sector, the Kuramoto-Einstein dictionary provides r = N (coherence = lapse) and K = G_gamma (spatial Green's function), so the horizon identification is derived there. The open question is whether this specific lattice system admits the same mapping. The Floquet reduction is a paper-sized computation.",
            quantity: "T_Hawking",
            predicted_value: "(hbar / 2 pi k_B) * |d rho / d Omega|_{tongue edge}",
            source_derivation: "analogue_horizon → arnold_tongues; cf. proslambenomenos/kuramoto_einstein_mapping.md for gravity-sector derivation",
            testable_by: "Derive circle-map reduction of the non-Hermitian lattice Hamiltonian; compare T_H from both formalisms",
            parent_slugs: &["analogue-horizon", "rotation-number", "arnold-tongues"],
        },
        PredictionEntry {
            slug: "circuit-depth-sb-depth",
            name: "Quantum circuit depth and Stern-Brocot depth",
            description: "Open question: does the point-vortex Hamiltonian spectrum exhibit rotation-number structure (as KAM theory would suggest for the system Wang et al. (2506.04023) simulate)? If so, minimum circuit depth should scale with depth_SB(p/q). Classical parameter resolution cost grows exponentially with SB depth (window width ~ 0.458^depth at K=1.5, confirmed by computation). The quantum half — whether phase estimation achieves polynomial overhead — remains to be computed.",
            quantity: "D_circuit",
            predicted_value: "depth_SB(p/q) where |rho - p/q| < epsilon",
            source_derivation: "quantum_vortex_simulation → stern_brocot_tree; classical cost scaling confirmed by computation",
            testable_by: "Establish rotation-number structure in point-vortex Hamiltonian spectrum; benchmark circuit depth vs SB depth",
