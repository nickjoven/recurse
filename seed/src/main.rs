...
fn main() {
    ...
    let ontology: Vec<OntologyEntry> = vec![
        ...
        OntologyEntry {
            ...
            description: "Coherent beam combining produces high-power charge-ℓ vortices with efficiency decreasing with ℓ (Fathi et al. 2512.19815: 95.0%, 93.9%, 91.2% at ℓ=1,5,8). Polygonal STOVs with prescribed geometries exist (Fan et al. 2512.16308). At K > 1, the grammar predicts multistability (coexisting OAM states) and tongue-internal cascades. The relationship between beam combining efficiency and tongue-width scaling has not been established. K(phase_noise, N_beams, ℓ) for coherent beam arrays remains to be derived.",
            symbol: None,
    ...
    let predictions: Vec<PredictionEntry> = vec![
        ...
        PredictionEntry {
            ...
            name: "Hawking temperature from tongue-edge gradient [OPEN: lattice reduction needed]",
            description: "Predicts T_H = (ℏ/2πk_B) · |dρ/dΩ| at the tongue edge. Requires deriving the circle-map reduction of Munoz-Arboleda et al.'s non-Hermitian tight-binding Hamiltonian (2507.03826). Plausible via Floquet analysis of the non-Hermitian system, but the reduction is a paper-sized problem. Note: in the gravity sector, the Kuramoto-Einstein dictionary provides r = N (coherence = lapse) and K = G_γ (spatial Green's function), so the horizon identification IS derived there. The open question is whether Paper 1's specific lattice system admits the same mapping.",
            quantity: "T_Hawking",
        ...
        PredictionEntry {
            ...
            name: "Quantum circuit depth scales with Stern-Brocot depth [OPEN: KAM bridge needed]",
            description: "Predicts minimum circuit depth ∝ depth_SB(p/q) for target rotation number precision. Requires proving that point-vortex Hamiltonian eigenvalues are organized by rational rotation numbers — a KAM theory question for the specific system Wang et al. (2506.04023) simulate. Computation confirms that classical parameter resolution cost grows exponentially with SB depth (window width ~ 0.458^depth at K=1.5). The quantum half — polynomial overhead via phase estimation — is plausible but uncomputed.",
            quantity: "D_circuit",
